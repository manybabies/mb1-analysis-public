...
```{r}
d <- exclude_by(d, quo(second_session))
diffs <- exclude_by(diffs, quo(second_session))
```

and write: 

```{r}
write_csv(d, "processed_data/03_data_trial_no2ndsess.csv")
write_csv(diffs, "processed_data/03_data_diff_no2ndsess.csv")
```